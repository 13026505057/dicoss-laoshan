                 ...
                 <el-table
                  ...
                  <el-table-column
                    ...
                    <template slot-scope="props">
                      ...
                      <el-button  type="warning" size="mini" style="margin-left: 20px;" @click="printClick(props.row)">新增案卷</el-button>
  ...
  export default {
      ...
      methods: {
          ...
          addNewJuanzong(){
                var self = this;
                var params = new URLSearchParams();
                if(self.addNewForm.niandu>2020||self.addNewForm.niandu<1950){
                      self.$message({
                        type: 'error',
                        message: '年份错误'
                      });
                ...
                var token = localStorage.getItem('auth');
                localStorage.setItem('nd',self.addNewForm.niandu)
          ...
          printClick(res){
            ...
            var s2 = day2.getFullYear()-1;
            var niandu =  localStorage.getItem('nd');
            if(niandu&&niandu>1000){
              s2 = niandu;
            }
            var xianyiren = '';
            var params = new URLSearchParams();
            ...
            params.append('tysah',res.tysah);

            self.$axios({
                method: 'post',
                url: '/casesFzxyrGetByPage',
                data: params,
                headers: {'Content-Type': 'application/x-www-form-urlencoded','kf-token':token},
             }).then(function(data){
                ...
                if(data.data.code==0){
                  if(data.data.data.list.length>0){
                    for(var j = 0; j < data.data.data.list.length; j++){
                      xianyiren = xianyiren+data.data.data.list[j].xm+','
                    }
                    self.addNewForm = {
                        juanzongType:'SS',
                        timeType:'1',
                    ...
                    self.addNewAnJuan = true;
                    self.print_accept = false;
          ...
          getDataList(){
                ...
                self.$axios({
                    method: 'post',
                    url: '/cases/cases/getConfirmedByPage',
                    data: params,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded','kf-token':token},
                 }).then(function(data){
